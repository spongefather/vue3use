...
export function initWorldWind(canvasid, range) {
  ...
  requestWms(url,'countries', wwd, 2)
  requestWms(url,'populated_places', wwd, 3)
  requestWms(url,'coastlines', wwd, 4)
  requestWms(url,'boundary_lines', wwd, 5)
  return { engine: wwd , interval}
}

function requestWms(url, name, wwd, index) {
  fetch(url)
  .then(resp => resp.text())
  .then(text => {
    // TODO ...
  ...
  .then(
    dom => {
      ...
      wwd.insertLayer(index, wmsLayer)
    }
  ).catch(reason => {
    console.log(reason)
  })
